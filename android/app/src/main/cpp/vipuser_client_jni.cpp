...
VipUserClientJni::VipUserClientJni(): _client(NULL)
{
}
...
void VipUserClientJni::init(const std::string & svraddr, const std::string & pubkey)
{
    if (_client == NULL) {
        _client = new VipUserClient(svraddr, pubkey);
    }
}
